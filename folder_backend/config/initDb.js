...
async function initializeDatabase() {
  ...
  try {
    ...
    // Tambah kolom is_active jika tabel sudah ada tapi kolom belum ada
    // (ALTER TABLE aman dipanggil berulang karena IF NOT EXISTS)
    await client.query(`
      ALTER TABLE certificates ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true
    `);
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS contacts (
        ...
        location VARCHAR(50) DEFAULT 'both', -- Lokasi tampil (footer/page/both)
        created_at TIMESTAMP DEFAULT NOW()  -- Waktu dibuat
      )
    `);
