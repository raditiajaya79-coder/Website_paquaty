...
const ManageEvents = () => {
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            ...
            <div className="divide-y divide-slate-100/60 pb-12 bg-white border border-slate-200 rounded-[2rem] shadow-sm overflow-hidden">
                {loading ? (
                    ...
                    <motion.div
                        ...
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className={`group p-5 md:p-6 flex flex-col md:flex-row md:items-center gap-5 sm:gap-6 transition-all hover:bg-slate-50/80 ${item.is_pinned ? 'bg-blue-50/30' : 'bg-transparent'}`}
                    >
                        <div className="flex items-start md:items-center gap-5 flex-1">
                            {/* Type Icon */}
                            <div className={`w-12 h-12 sm:w-14 sm:h-14 rounded-2xl flex items-center justify-center shrink-0 border shadow-sm group-hover:scale-110 transition-transform duration-300 ${item.type === 'Article' ? 'bg-indigo-50 border-indigo-100 text-indigo-500' : 'bg-emerald-50 border-emerald-100 text-emerald-500'}`}>
                                {item.type === 'Article' ? <FileText className="w-5 h-5 sm:w-6 sm:h-6" /> : <Calendar className="w-5 h-5 sm:w-6 sm:h-6" />}
                            </div>
                        ...
                        {/* Actions */}
                        <div className="flex items-center gap-3 md:opacity-0 group-hover:opacity-100 transition-opacity justify-end border-t border-slate-100 md:border-t-0 pt-4 md:pt-0 mt-2 md:mt-0 px-2 md:px-0">
                            <button
                            ...
                            <div className="p-2 text-slate-200 hidden md:block group-hover:text-slate-300 transition-colors">
